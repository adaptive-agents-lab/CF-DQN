...
def gaussian_collapse_q_values(omega_grid, V_complex, n_pairs=5):
    """
    Extracts Q-values from the CF by using the Imaginary component near ω→0.
    Since V(ω) = E[cos(ωZ)] + i E[sin(ωZ)], for small ω, Imag(V) ≈ ω * E[Z] = ω * Q.
    This completely avoids torch.angle(), phase wrapping, and unwrapping artifacts.
    """
    ...
    # Extract the raw Imaginary component directly (NO torch.angle)
    imag_pos = V_complex[..., pos_idx].imag
    imag_neg = V_complex[..., neg_idx].imag

    # Q ≈ (Imag(+w) - Imag(-w)) / (2w)
    slopes = (imag_pos - imag_neg) / (2.0 * omega_pos)   
...
def polar_interpolation(omega_grid, target_V_complex, gammas):
    """
    Interpolates the target network's CF at the scaled frequencies (gamma * omega).
    Uses direct complex linear interpolation to completely avoid torch.angle() and wrapping artifacts.
    ...
    # Linearly interpolate the complex numbers directly
    V_left = target_V_complex[batch_idx, idx_left]
    V_right = target_V_complex[batch_idx, idx_right]
    
    interp_V = (1 - t) * V_left + t * V_right
